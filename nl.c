...
struct msghdr {
    void         *msg_name;
    socklen_t    msg_namelen;  //套接口地址成员 ,套接口是一个数据报套接口时才需要,指向我们要发送或是接收信息的套接口地址
    struct iovec *msg_iov;
    size_t       msg_iovlen; //IO向量引用
    void         *msg_control;
    size_t       msg_controllen;  //附属数据缓冲区
    int          msg_flags;  //接受信息标记位

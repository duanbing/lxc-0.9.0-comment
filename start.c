...
static int must_drop_cap_sys_boot(void)
{
    ...
    //CLONE_NEWNET 是进程资源隔离的关键。如果设置此选项，将创建新的pid
    //namespace，container_reboot_supported将成为namespace的第一个进程,pid=1
#ifdef __ia64__
...
int lxc_spawn(struct lxc_handler *handler)
{
	...
	if (!lxc_list_empty(&handler->conf->network)) {
        ...
        //由于创建的新的container(命名空间内的进程组)无法访问外界的资源， 因此先获得这些信息带进container
